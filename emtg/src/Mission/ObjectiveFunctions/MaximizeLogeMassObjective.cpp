    ...
    namespace ObjectiveFunctions
{
        ...
        MaximizeLogeMassObjective::MaximizeLogeMassObjective(Astrodynamics::universe* Universe,
            ...
            ObjectiveFunctionBase(Universe,
                ...
                myMission)
        {
            this->name = "MaximizeLogeMassObjectiveFunction";
        ...
        void MaximizeLogeMassObjective::calcbounds()
        {
        ...
        void MaximizeLogeMassObjective::process(const std::vector<doubleType>& X,
            size_t& Xindex,
            ...
            const bool& needG)
        {
            ...
            this->ObjectiveValue = -log((FinalState(6) + 1.0) * this->myOptions->Journeys.back().maximum_mass);
            ...
            if (needG)
            {
                ...
                for (size_t dIndex = 0; dIndex < this->number_of_non_time_derivatives; ++dIndex)
                {
                    ...
                    G[Gindex] = this->X_scale_factors->operator[](Xindex)
                        * -1.0 / (FinalState(6) + 1.0)_GETVALUE
        ...
        void MaximizeLogeMassObjective::output(std::ofstream& outputfile)
        {
            outputfile << "Objective function is \"maximize log(final mass)\"" << std::endl;
