...
namespace EMTG
{
    namespace BoundaryEvents
    {
        namespace SpecializedConstraints
        {
            ...
            RPBconstraint::RPBconstraint(const std::string& name,
                    ...
                    constraintDefinition)
            {
                ...
                this->dcosRPB_dR_body_reference = math::Matrix<double>(3, 1, 0.0);
                this->dcosRPB_dV_body_probe = math::Matrix<double>(3, 1, 0.0);
            ...
            void RPBconstraint::calcbounds()
            {
                // Step 1: parse the constraint definition
                // constraintDefinition is like p0_arrival_RPB_cb_0.0_10.0 (numbers are in degrees)
                ...
                if (ConstraintDefinitionCell[1] != "arrival")
                    throw std::invalid_argument("RPB constraint may only be applied to boundary events of class EphemerisPeggedArrivalWithVinfinity. Place a breakpoint in " + std::string(__FILE__) + ", line " + std::to_string(__LINE__));
                ...
                this->Fdescriptions->push_back(prefix + this->referenceName + "-Probe-Body constraint");
            ...
            void RPBconstraint::process_constraint(const std::vector<doubleType>& X,
                size_t& Xindex,
                ...
                const bool& needG)
            {
                ...
                for (size_t stateIndex : {0, 1, 2})
                {
                    this->V_body_probe(stateIndex) = Vinfinity(stateIndex);
                }

                //Step 4: compute the angle between the vectors
                this->cosRPB = this->R_body_reference.dot(this->V_body_probe) / (this->R_body_reference.norm() * this->V_body_probe.norm());
                this->Angle = math::safe_acos(this->cosRPB);

                //Step 5: apply the constraint - we will constrain the cos(Angle) instead of the Angle itself
                F[Findex++] = this->cosRPB;
                ...
                if (needG)
                {
                    ...
                    this->dcosRPB_dR_body_reference(0) = vx / (r * v) - (rx * vx + ry * vy + rz * vz) * dr_body_reference_drx / (r * r * v);
                    this->dcosRPB_dR_body_reference(1) = vy / (r * v) - (rx * vx + ry * vy + rz * vz) * dr_body_reference_dry / (r * r * v);
                    this->dcosRPB_dR_body_reference(2) = vz / (r * v) - (rx * vx + ry * vy + rz * vz) * dr_body_reference_drz / (r * r * v);

                    this->dcosRPB_dV_body_probe(0) = rx / (r  *v) - (rx * vx + ry * vy + rz * vz) * dv_body_probe_dvx / (r * v * v);
                    this->dcosRPB_dV_body_probe(1) = ry / (r * v) - (rx * vx + ry * vy + rz * vz) * dv_body_probe_dvy / (r * v * v);
                    this->dcosRPB_dV_body_probe(2) = rz / (r * v) - (rx * vx + ry * vy + rz * vz) * dv_body_probe_dvz / (r * v * v);
                    ...
                    for (size_t stateIndex = 0; stateIndex < 3; ++stateIndex)
                    {
                        for (size_t entryIndex = 0; entryIndex < this->dIndex_with_respect_to_PositionAfterEvent_wrt_Time[stateIndex].size(); ++entryIndex)
                        {
                            ...
                            double Gentry = this->dcosRPB_dR_body_reference(stateIndex) * std::get<2>(Derivatives_of_StateAfterEvent_wrt_Time[dIndex]);

                            if (!this->isCentralBodyConstraint)
                            {
                                Gentry -= this->dcosRPB_dR_body_reference(stateIndex) * this->dR_reference_cb_dt(stateIndex);
                            }
                    ...
                    for (size_t velocityIndex : {0, 1, 2})
                    {
                        ...
                        double Gentry = this->dcosRPB_dV_body_probe(velocityIndex);
            ...
            void RPBconstraint::output(std::ofstream& outputfile)
            {
                outputfile << this->myBoundaryEvent->getName() << " " << this->referenceName + "-Probe-Body angle (deg): " << this->Angle _GETVALUE * 180.0 / math::PI << std::endl;
            }//end output()
