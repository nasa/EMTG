...
namespace EMTG
{
    namespace BoundaryEvents
    {
        namespace SpecializedConstraints
        {
            ...
            BoundaryDeticLatitudeConstraint::BoundaryDeticLatitudeConstraint(const std::string& name,
            ...
            void BoundaryDeticLatitudeConstraint::calcbounds()
            {
                // Step 1: parse the constraint definition
				// constraintDefinition is like p0_arrival_DeticLatitude_30.0_30.1_J2000BCF
                ...
                this->Fdescriptions->push_back(prefix + "detic latitude");
            ...
            void BoundaryDeticLatitudeConstraint::process_constraint(const std::vector<doubleType>& X,
                size_t& Xindex,
                ...
                const bool& needG)
            {
                ...
                //Step 3: compute deticlatitude magnitude
				...
				if (rxy < math::SMALL)
				{
					throw std::runtime_error(this->prefix + "The probe is too close to the pole to calculate detic latitude and its derivatives.");
				}
				...
				this->DeticLatitude = mua - delmu;

                F[Findex++] = this->DeticLatitude;
                //Step 4: derivatives
                if (needG)
                {
					...
					double dmu_drx = (dmua_drx - ddelmu_drx) _GETVALUE;
					double dmu_dry = (dmua_dry - ddelmu_dry) _GETVALUE;
					double dmu_drz = (dmua_drz - ddelmu_drz) _GETVALUE;
					...
					for (size_t stateIndex = 0; stateIndex < 3; ++stateIndex)
					{
						...
						for (size_t entryIndex = 0; entryIndex < this->dIndex_with_respect_to_StateBeforeEvent[stateIndex].size(); ++entryIndex)
						{
							...
							double Gentry = ( dmu_drx * drxConstraintFrame_dstateICRF
											+ dmu_dry * dryConstraintFrame_dstateICRF
											+ dmu_drz * drzConstraintFrame_dstateICRF)
                    ...
                    double timeDerivative = ( dmu_drx * this->dPositionBeforeEventConstraintFrame_dt(0)
											+ dmu_dry * this->dPositionBeforeEventConstraintFrame_dt(1)
											+ dmu_drz * this->dPositionBeforeEventConstraintFrame_dt(2)) _GETVALUE;
            ...
            void BoundaryDeticLatitudeConstraint::output(std::ofstream& outputfile)
            {
                std::string framestring = this->myUniverse->central_body_name + " " + this->ReferenceFrameStrings[this->myReferenceFrame];

                outputfile << this->myBoundaryEvent->getName() << " detic latitude (degrees, " << framestring << "): " << this->DeticLatitude _GETVALUE / math::deg2rad << std::endl;
            }//end output()
