...
namespace EMTG
{
    namespace BoundaryEvents
    {
        namespace SpecializedConstraints
        {
            ...
            BoundaryRelativeVelocityMagnitudeConstraint::BoundaryRelativeVelocityMagnitudeConstraint(const std::string& name,
            ...
            void BoundaryRelativeVelocityMagnitudeConstraint::calcbounds()
            {
                // Step 1: parse the constraint definition
				// constraintDefinition is like p0_arrival_RelativeVMagnitude_11.9_12.1_J2000BCF
                ...
                this->Fdescriptions->push_back(prefix + "relative velocity magnitude");
            ...
            void BoundaryRelativeVelocityMagnitudeConstraint::process_constraint(const std::vector<doubleType>& X,
                size_t& Xindex,
                ...
                const bool& needG)
            {
                ...
                //Step 3: compute velocity magnitude
				...
				doubleType vrz = vz;
				this->RelativeVelocityMagnitude = sqrt(vrx * vrx + vry * vry + vrz * vrz);

                F[Findex++] = this->RelativeVelocityMagnitude;
                //Step 4: derivatives
                if (needG)
                {
                    ...
                    double vr = this->RelativeVelocityMagnitude _GETVALUE;
                    double dvr_dvx = (vrx / vr) _GETVALUE;
                    double dvr_dvy = (vry / vr) _GETVALUE;
                    double dvr_dvz = (vrz / vr) _GETVALUE;
					double dvr_drx = (- Wdot * vry / vr) _GETVALUE;
					double dvr_dry = (Wdot * vrx / vr) _GETVALUE;
					double dvr_drz = 0.0;
					...
					for (size_t stateIndex = 0; stateIndex < 3; ++stateIndex)
					{
						...
						for (size_t entryIndex = 0; entryIndex < this->dIndex_with_respect_to_StateBeforeEvent[stateIndex].size(); ++entryIndex)
						{
							...
							double Gentry = ( dvr_drx * drxConstraintFrame_dstateICRF
											+ dvr_dry * dryConstraintFrame_dstateICRF
											+ dvr_drz * drzConstraintFrame_dstateICRF)
                    ...
                    for (size_t stateIndex = 3; stateIndex < 6; ++stateIndex)
                    {
                        ...
                        for (size_t entryIndex = 0; entryIndex < this->dIndex_with_respect_to_StateBeforeEvent[stateIndex].size(); ++entryIndex)
                        {
                            ...
                            double Gentry = (  dvr_dvx * dvxConstraintFrame_dstateICRF 
                                             + dvr_dvy * dvyConstraintFrame_dstateICRF
                                             + dvr_dvz * dvzConstraintFrame_dstateICRF)
                                * std::get<2>(Derivatives_of_StateBeforeEvent[dIndex]);
                    ...
                    for (size_t stateIndex = 0; stateIndex < 3; ++stateIndex)
                    {
                        ...
                        for (size_t entryIndex = 0; entryIndex < this->dIndex_with_respect_to_StateBeforeEvent_wrt_Time[stateIndex].size(); ++entryIndex)
                        {
                            ...
                            double Gentry = (+ dvr_drx * drxConstraintFrame_dstateICRF
											 + dvr_dry * dryConstraintFrame_dstateICRF
											 + dvr_drz * drzConstraintFrame_dstateICRF 
											 + dvr_dvx * dvxConstraintFrame_dstateICRF
                                             + dvr_dvy * dvyConstraintFrame_dstateICRF
                                             + dvr_dvz * dvzConstraintFrame_dstateICRF)
                    ...
                    double timeDerivative = ( dvr_drx * this->dPositionBeforeEventConstraintFrame_dt(0)
											+ dvr_dry * this->dPositionBeforeEventConstraintFrame_dt(1)
											+ dvr_drz * this->dPositionBeforeEventConstraintFrame_dt(2)
											+ dvr_dvx * this->dVelocityBeforeEventConstraintFrame_dt(0)
											+ dvr_dvy * this->dVelocityBeforeEventConstraintFrame_dt(1)
											+ dvr_dvz * this->dVelocityBeforeEventConstraintFrame_dt(2)) _GETVALUE;
            ...
            void BoundaryRelativeVelocityMagnitudeConstraint::output(std::ofstream& outputfile)
            {
                std::string framestring = this->myUniverse->central_body_name + " " + this->ReferenceFrameStrings[this->myReferenceFrame];

                outputfile << this->myBoundaryEvent->getName() << " relative velocity magnitude (km/s, " << framestring << "): " << this->RelativeVelocityMagnitude _GETVALUE  << std::endl;
            }//end output()
