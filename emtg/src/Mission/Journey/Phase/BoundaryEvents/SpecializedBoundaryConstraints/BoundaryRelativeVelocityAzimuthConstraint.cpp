...
namespace EMTG
{
    namespace BoundaryEvents
    {
        namespace SpecializedConstraints
        {
            ...
            BoundaryRelativeVelocityAzimuthConstraint::BoundaryRelativeVelocityAzimuthConstraint(const std::string& name,
            ...
            void BoundaryRelativeVelocityAzimuthConstraint::calcbounds()
            {
                // Step 1: parse the constraint definition
				// constraintDefinition is like p0_arrival_RelativeVAzimuth_-10.9_-11.0_trueofdatebcf
                ...
                this->Fdescriptions->push_back(prefix + "relative velocity Azimuth");
            ...
            void BoundaryRelativeVelocityAzimuthConstraint::process_constraint(const std::vector<doubleType>& X,
                size_t& Xindex,
                ...
                const bool& needG)
            {
				...
				if (rxy < math::SMALL)
				{
					throw std::runtime_error(this->prefix + "The probe is too close to the pole to make detic local coorinate for Azimuth.");
				}
				...
				this->RelativeVelocityAzimuth = atan2(ve, vn);

                F[Findex++] = this->RelativeVelocityAzimuth;
                //Step 4: derivatives
                if (needG)
                {
					...
					double vne = (sqrt(vn * vn + ve * ve)) _GETVALUE;
					double vne2 = (vn / vne / vne) _GETVALUE;
            ...
            void BoundaryRelativeVelocityAzimuthConstraint::output(std::ofstream& outputfile)
            {
                std::string framestring = this->myUniverse->central_body_name + " " + this->ReferenceFrameStrings[this->myReferenceFrame];

                outputfile << this->myBoundaryEvent->getName() << " relative velocity Azimuth (degrees, " << framestring << "): " << this->RelativeVelocityAzimuth _GETVALUE / math::deg2rad << std::endl;
            }//end output()
