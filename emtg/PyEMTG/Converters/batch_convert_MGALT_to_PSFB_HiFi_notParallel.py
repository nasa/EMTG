...
if len(sys.argv) != 4:
    raise Exception("Unknown number of command line options!\n\
                     Syntax: python batch_convert_MGALT_to_PSFB_HiFi.py source_directory destination_directory path_to_EMTG")
...
for root,dirs,files in os.walk(destination_dir + '/temp_high_fidelity/'):
    for file in files:
        if file.endswith('.emtg'):
            ...
            newOptions.mission_name = missionFile.replace('.emtg','').replace('MGALT','PSFB').replace('FAILURE_','')
