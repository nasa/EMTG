...
namespace EMTG
{
    ...
    {
        //constructors
        SphericalHarmonicTerm::SphericalHarmonicTerm(SpacecraftAccelerationModel * acceleration_model_in, CentralBodyGravityTerm * parent_gravity_term_in) :
                               ...
                               parent_gravity_term(parent_gravity_term_in)
        {
            ...
            this->accel_position_Jacobian_ICRF.resize(3, 3, 0.0);
        }
        ...
        {
            this->computeBodyFixedPosition();

            this->parent_gravity_term->my_body->harmonic_gravity_field->computeHarmonicGravityField(this->r_body2sc_BCF, false);
            this->spherical_harmonic_term_acceleration_BCF = this->parent_gravity_term->my_body->harmonic_gravity_field->getBCFaccel();
            this->computeInertialAcceleration();
        }
        ...
        void SphericalHarmonicTerm::computeAccelerationTerm(const bool& generate_derivatives)
        {
            ...
            this->parent_gravity_term->my_body->harmonic_gravity_field->computeHarmonicGravityField(this->r_body2sc_BCF, generate_derivatives);
            this->spherical_harmonic_term_acceleration_BCF = this->parent_gravity_term->my_body->harmonic_gravity_field->getBCFaccel();
            this->accel_position_Jacobian_BCF = this->parent_gravity_term->my_body->harmonic_gravity_field->getBCFpositionJacobian();
            this->computeInertialAcceleration();
            this->populateStatePropagationMatrix();
        }

        void SphericalHarmonicTerm::computeBodyFixedPosition()
        {
            ...
            this->acceleration_model->my_universe->LocalFrame.rotate_frame_to_frame(ReferenceFrame::ICRF,
                                                                                    temp_matrix_in,
                                                                                    ReferenceFrame::TrueOfDate_BCF,
                                                                                    temp_matrix_out,
        ...
        void SphericalHarmonicTerm::computeInertialAcceleration()
        {
            static EMTG::math::Matrix <doubleType> temp_matrix_in(3, 1, 0.0);
            static EMTG::math::Matrix <doubleType> temp_matrix_out(3, 1, 0.0);
            ...
            this->acceleration_model->my_universe->LocalFrame.rotate_frame_to_frame(ReferenceFrame::TrueOfDate_BCF,
                                                                                    temp_matrix_in,
                                                                                    ReferenceFrame::ICRF,
                                                                                    temp_matrix_out,
                                                                                    this->acceleration_model->current_epoch);
        ...
        void SphericalHarmonicTerm::populateStatePropagationMatrix()
        {
            ...
            this->acceleration_model->my_universe->LocalFrame.construct_rotation_matrices(this->acceleration_model->current_epoch, true);
            ...
            this->accel_position_Jacobian_ICRF = (R_from_BCF_to_ICRF * this->accel_position_Jacobian_BCF * R_from_ICRF_to_BCF);
            ...
            d_spherical_accel_ICRF_dt = (dR_from_BCF_to_ICRF_dt * this->spherical_harmonic_term_acceleration_BCF
                + R_from_BCF_to_ICRF * this->accel_position_Jacobian_BCF * dR_from_ICRF_to_BCF_dt * this->parent_gravity_term->r_body2sc);
        ...
        void SphericalHarmonicTerm::computeJ2Acceleration()
        {
            // J2 calculations must be performed in the central-body fixed equatorial frame (BCF)
            this->computeBodyFixedPosition();

            // compute the acceleration due to central body oblateness
            ...
            doubleType J2coeff = 3.0 * this->acceleration_model->my_universe->central_body_J2 * this->mu * this->harmonic_field_reference_radius * this->harmonic_field_reference_radius / (2.0 * spacecraft_distance_from_central_body_BCF5);
        ...
        }// end computeJ2Acceleration()
        

        
        void SphericalHarmonicTerm::computeJ2Acceleration(const bool & generate_derivatives)
        {
            this->computeJ2Acceleration();
            ...
            doubleType J2_deriv_coeff = -this->acceleration_model->my_universe->central_body_J2 * this->mu * this->harmonic_field_reference_radius * this->harmonic_field_reference_radius / (2.0 * spacecraft_distance_from_central_body_BCF9);
            doubleType J2_deriv_coeff3 = J2_deriv_coeff * J2_deriv_coeff * J2_deriv_coeff;
            
            this->accel_position_Jacobian_BCF(0, 0) = -3.0  * J2_deriv_coeff * (4.0 * x4 + 3.0 * x2y2 - 27.0* x2z2 - y4 + 3.0 * y2z2 + 4.0 * z4);
            this->accel_position_Jacobian_BCF(0, 1) = -15.0 * J2_deriv_coeff * xy * (x2 + y2 - 6.0 * z2);
            this->accel_position_Jacobian_BCF(0, 2) = -15.0 * J2_deriv_coeff * xz * (3.0 * x2 + 3.0 * y2 - 4.0 * z2);
            this->accel_position_Jacobian_BCF(1, 0) = -15.0 * J2_deriv_coeff * xy * (x2 + y2 - 6.0 * z2);
            this->accel_position_Jacobian_BCF(1, 1) = -3.0  * J2_deriv_coeff * (-1.0 * x4 + 3.0 * x2y2 + 3.0 * x2z2 + 4.0 * y4 - 27.0 * y2z2 + 4.0 * z4);
            this->accel_position_Jacobian_BCF(1, 2) = -15.0 * J2_deriv_coeff * yz * (3.0 * x2 + 3.0 * y2 - 4.0 * z2);
            this->accel_position_Jacobian_BCF(2, 0) = -15.0 * J2_deriv_coeff * xz * (3.0 * x2 + 3.0 * y2 - 4.0 * z2);
            this->accel_position_Jacobian_BCF(2, 1) = -15.0 * J2_deriv_coeff * yz * (3.0 * x2 + 3.0 * y2 - 4.0 * z2);
            this->accel_position_Jacobian_BCF(2, 2) = 3.0  * J2_deriv_coeff * (3.0 * x4 + 6.0 * x2y2 - 24.0 * x2z2 + 3.0 * y4 - 24.0 * y2z2 + 8.0 * z4);

            this->populateStatePropagationMatrix();
            
        }// end computeJ2Acceleration(bool)
