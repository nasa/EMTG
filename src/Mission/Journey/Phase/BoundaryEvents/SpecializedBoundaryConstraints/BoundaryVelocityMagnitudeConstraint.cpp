...
namespace EMTG
{
    namespace BoundaryEvents
    {
        namespace SpecializedConstraints
        {
            ...
            BoundaryVelocityMagnitudeConstraint::BoundaryVelocityMagnitudeConstraint(const std::string& name,
            ...
            void BoundaryVelocityMagnitudeConstraint::calcbounds()
            {
                // Step 1: parse the constraint definition
				// constraintDefinition is like p0_arrival_velocitymagnitude_11.9_12.1_J2000BCF
                ...
                this->Fdescriptions->push_back(prefix + "velocity magnitude");
            ...
            void BoundaryVelocityMagnitudeConstraint::process_constraint(const std::vector<doubleType>& X,
                size_t& Xindex,
                ...
                const bool& needG)
            {
                ...
                this->VelocityMagnitude = this->VelocityBeforeEventConstraintFrame.norm();

                F[Findex++] = this->VelocityMagnitude;
                ...
                if (needG)
                {
                    ...
                    double v = this->VelocityMagnitude _GETVALUE;
                    ...
                    for (size_t stateIndex = 0; stateIndex < 3; ++stateIndex)
                    {
                        ...
                        for (size_t entryIndex = 0; entryIndex < this->dIndex_with_respect_to_StateBeforeEvent[stateIndex].size(); ++entryIndex)
                        {
                            ...
                            double Gentry = (  dv_dvx * dvxConstraintFrame_dstateICRF 
                                             + dv_dvy * dvyConstraintFrame_dstateICRF
                                             + dv_dvz * dvzConstraintFrame_dstateICRF)
                    ...
                    double timeDerivative = (dv_dvx * this->dVelocityBeforeEventConstraintFrame_dt(0)
                        + dv_dvy * this->dVelocityBeforeEventConstraintFrame_dt(1)
                        + dv_dvz * this->dVelocityBeforeEventConstraintFrame_dt(2)) _GETVALUE;
            ...
            void BoundaryVelocityMagnitudeConstraint::output(std::ofstream& outputfile)
            {
                std::string framestring = this->myUniverse->central_body_name + " " + this->ReferenceFrameStrings[this->myReferenceFrame];

                outputfile << this->myBoundaryEvent->getName() << " velocity magnitude (km/s, " << framestring << "): " << this->VelocityMagnitude _GETVALUE  << std::endl;
            }//end output()
