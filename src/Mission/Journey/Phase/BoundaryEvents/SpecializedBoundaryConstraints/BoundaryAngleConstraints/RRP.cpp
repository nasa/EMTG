    ...
    namespace BoundaryEvents
    {
        namespace SpecializedConstraints
        {
                ...
                const bool& needG)
            {
                ...
                for (size_t stateIndex : {0, 1, 2})
                {
                    this->R_ref2_probe(stateIndex) = SpacecraftStateRelativeToCentralBody(stateIndex) - this->R_cb_ref2(stateIndex);
                }
                ...
                if (needG)
                {
                    ...
                    for (size_t stateIndex = 0; stateIndex < 3; ++stateIndex)
                    {
                        for (size_t entryIndex = 0; entryIndex < this->dIndex_with_respect_to_PositionAfterEvent_wrt_Time[stateIndex].size(); ++entryIndex)
                        {
                            ...
                            double Gentry = this->dcosRRP_dR_ref2_probe(stateIndex) * (std::get<2>(Derivatives_of_StateAfterEvent_wrt_Time[dIndex]) - this->dR_cb_ref2_dt(stateIndex))
