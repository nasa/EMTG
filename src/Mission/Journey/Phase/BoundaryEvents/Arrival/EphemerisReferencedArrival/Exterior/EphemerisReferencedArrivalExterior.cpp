...
namespace EMTG
{
    ...
    {
        EphemerisReferencedArrivalExterior::EphemerisReferencedArrivalExterior(const std::string& name,
        ...
        {
            this->EphemerisReferencedArrivalExterior::initialize(name,
                journeyIndex,
                phaseIndex,
                stageIndex,
                Universe,
        ...
        void EphemerisReferencedArrivalExterior::initialize(const std::string& name,
            const size_t& journeyIndex,
            const size_t& phaseIndex,
            size_t& stageIndex,
            Astrodynamics::universe* Universe,
            ...
            missionoptions* myOptions)
        {
            ...
            if (this->myJourneyOptions->sequence[phaseIndex + 1] == 0) //landing on the central body
            {
				...
				{
					this->semi_axis_a = this->myBody->radius;
					this->semi_axis_b = this->myBody->radius;
					this->semi_axis_c = this->myBody->radius;

					std::cout << "User inputed 0.0 for journey arrival ellipsoid axes in journey " + std::to_string(this->journeyIndex) + ". This instructs EMTG to use the body's radius of " + std::to_string(this->myBody->r_SOI) + " km." << std::endl;
				}
            }
            else //body in the universe
            {
                this->myBody = &myUniverse->bodies[this->myJourneyOptions->sequence[phaseIndex + 1] - 1];
                this->semi_axis_a = this->myJourneyOptions->arrival_ellipsoid_axes[0];
                this->semi_axis_b = this->myJourneyOptions->arrival_ellipsoid_axes[1];
                this->semi_axis_c = this->myJourneyOptions->arrival_ellipsoid_axes[2];
        ...
        //******************************************calcbounds methods
        void EphemerisReferencedArrivalExterior::calcbounds_event_left_side(std::vector<size_t> timeVariables)
        {
        ...
        void EphemerisReferencedArrivalExterior::calcbounds_event_right_side()
        {
            //in an EphemerisReferencedArrivalExterior, the right-hand side is identical to the left-hand side, minus the state of the body
        ...
        //**************************************process functions
        void EphemerisReferencedArrivalExterior::process_event_left_side(const std::vector<doubleType>& X,
                                                                         size_t& Xindex,
        ...
        void EphemerisReferencedArrivalExterior::process_event_right_side(const std::vector<doubleType>& X,
                                                                          size_t& Xindex,
                                                                          std::vector<doubleType>& F,
                                                                          size_t& Findex,
                                                                          std::vector<double>& G,
