...
#include "EphemerisReferencedInterceptInterior.h"

namespace EMTG
{
    namespace BoundaryEvents
    {
        EphemerisReferencedInterceptInterior::EphemerisReferencedInterceptInterior(const std::string& name,
            ...
            EphemerisReferencedArrivalWithVinfinityInterior::EphemerisReferencedArrivalWithVinfinityInterior(name,
                journeyIndex,
                phaseIndex,
                stageIndex,
                Universe,
        ...
        void EphemerisReferencedInterceptInterior::calcbounds_virtual_propellant_constraints()

        {
         ...
         void EphemerisReferencedInterceptInterior::process_event(const std::vector<doubleType>& X,
             size_t& Xindex,
         ...
         void EphemerisReferencedInterceptInterior::process_event_main(const std::vector<doubleType>& X,
                                                                       size_t& Xindex,
                                                                       std::vector<doubleType>& F,
                                                                       size_t& Findex,
                                                                       std::vector<double>& G,
         ...
         void EphemerisReferencedInterceptInterior::process_virtual_propellant_constraints(const std::vector<doubleType>& X,
             size_t& Xindex,
             std::vector<doubleType>& F,
             size_t& Findex,
             std::vector<double>& G,
         ...
         //******************************************output methods
         void EphemerisReferencedInterceptInterior::output(std::ofstream& outputfile,
             ...
             size_t& eventcount)
         {
             ...
             else
             {
                 ...
                 //note that if this is not the last journey, we actually care about the next universe's central body
                 ...
                 math::Matrix<doubleType> R_Sun_CB(3, 1, 0.0);
                 for (size_t stateIndex = 0; stateIndex < 3; ++stateIndex)
                 {
                     R_Sun_CB(stateIndex) = central_body_state_and_derivatives[stateIndex];
                 }

                 R_sc_Sun = this->state_before_event.getSubMatrix1D(0, 2) + R_Sun_CB;
             }
